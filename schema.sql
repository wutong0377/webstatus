...
CREATE TABLE IF NOT EXISTS `status_history` (
  ...
  `dns_ms`       INT             NOT NULL DEFAULT 0 COMMENT 'DNS解析耗时(ms)',
  `tcp_ms`       INT             NOT NULL DEFAULT 0 COMMENT 'TCP握手耗时(ms)',
  `tls_ms`       INT             NOT NULL DEFAULT 0 COMMENT 'TLS握手耗时(ms)',
  `ttfb_ms`      INT             NOT NULL DEFAULT 0 COMMENT 'TTFB首字节耗时(ms)',
