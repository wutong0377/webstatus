...
/* ------------------- 安装向导（无 Origin 同源校验） ------------------- */
/**
 * v2.3.0 起移除安装阶段的 Origin/Host 同源校验：
 * 该校验在 Nginx / 宝塔 / CDN(Cloudflare) 等反向代理环境下，浏览器 Origin 与 Node 侧
 * Host 头经常不一致（代理改写 Host 为内网地址、或 CDN 转发），连续两次导致合法安装被
 * 误拦截（"来源校验失败"）。安装流程是一次性、登录前的流程，且要求管理员提供数据库
 * 凭据，跨站 CSRF 诱导完成安装的风险极低；为不影响安装可用性，此处不再拦截。
 * 如后续需要加固，请使用 session CSRF Token 机制（参考 middleware/csrf.js），
 * 而非基于 Origin/Host 的判断。
 */
